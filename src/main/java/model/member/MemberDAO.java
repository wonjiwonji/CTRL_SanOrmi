...
public class MemberDAO {
	...
	// UPDATE_MEMBER_BOARDCNT ; 본인이 쓴 게시글 갯수 변경
	...
	// SELECTALL_COMMUNITYKING ; 커뮤니티왕 조회
	final String SELECTALL_COMMUNITYKING = "SELECT ID, M_BOARDCNT FROM MEMBER ORDER BY M_BOARDCNT DESC LIMIT 0,5";
	// LOGIN_MEMBER ; 로그인
	...
	public boolean insertKakaoMember(MemberVO mvo) { // mvo ; Id, mName, mEmail 필요
		...
		try {
			pstmt = conn.prepareStatement(INSERT_KAKAO_MEMBER); // INSERT_KAKAO_MEMBER ; 카카오 회원 가입
	...
	public boolean insertNaverMember(MemberVO mvo) { // mvo ; Id, mName, mEmail 필요
		...
		try {
			pstmt = conn.prepareStatement(INSERT_NAVER_MEMBER);// INSERT_NAVER_MEMBER ; 카카오 회원 가입
	...
	public boolean updateMember(MemberVO mvo) { // mvo ; Id, mPW, mEmail, mAddress, mName, mDate 필요
		...
		try {
			...
			pstmt.setString(9, mvo.getId()); // 아이디
	...
	public ArrayList<MemberVO> selectAllMember(MemberVO mvo) { // Input 필요 없음
	...
	public ArrayList<MemberVO> selectAllCommunityKing(MemberVO mvo) {
		...
		try {
			pstmt = conn.prepareStatement(SELECTALL_COMMUNITYKING); // SELECTALL_MEMBER ; 전체 회원 보기
	...
	public MemberVO loginMember(MemberVO mvo) { // mvo ; Id, Pw 필요
		...
		try {
			...
			if (rs.next()) { // 저장할 정보가 남아 있는 동안
				data = new MemberVO(); // 새로운 MemberVO 객체 data 생성해서 다음 결과 set
