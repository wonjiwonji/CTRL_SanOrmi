...
public class QNADAO {
	...
	final String SELECTONE_QNA = "SELECT Q_ID, Q.Q_TITLE, Q.Q_CONTENT, Q.Q_DATE,\r\n"
			+ " COUNT(QC.QC_NUM) AS C_CNT, Q.Q_CNT\r\n"
			+ "FROM QNA Q LEFT JOIN QCOMMENT QC ON Q.Q_NUM = QC.Q_NUM WHERE Q.Q_NUM =? GROUP BY QC_NUM;";
	// SELECTALL_QNA; QNA 게시글 전체보기 쿼리문 ( 게시글 목록 시 나오는 정보 )
	final String SELECTALL_QNA = "SELECT Q_NUM, Q_TITLE, Q_CNT, Q_ID, Q_CONTENT,\r\n"
			+ " C_CNT FROM QNA ORDER BY Q_NUM DESC";
	// INSERT_QCOMMENT; 댓글 등록 쿼리문
	final String INSERT_QCOMMENT = "INSERT INTO QCOMMENT (QC_ID, Q_NUM,QC_CONTENT,QC_GROUP, QC_DATE) VALUES(?,?,?,(SELECT COALESCE(MAX(QC_GROUP),0)+1\r\n"
			+ " FROM QCOMMENT AS QC_GROUP),NOW())";
